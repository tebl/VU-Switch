...
Going forward with this build, I'll be assuming you bought the same version of VU meters and case. Substutute as needed.
...
# 1> VU Switch
Assembling the VU Switch is mainly just following the BOM below, soldering them into place in the order of how physically large the components are. What that means is install the diodes and resistors first, followed by JST-XH connectors (see NB on 1,1 first) and working your way up from there. Revision A is shown below, the later has some minor changes to the form factor - it is functionally the same.
...
## 1.2> Mounting the VU driver board
...
## 1.3> Drilling holes in the case
